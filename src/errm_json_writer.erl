...
encode_value(Atom, Order)  when is_atom(Atom)   -> encode_value(atom_to_binary(Atom, utf8), Order);
...
-spec build_pairs([any()], map(), undefined | [atom() | binary()], [iolist()]) -> [iolist()].
build_pairs([], _Map, _Order, Acc) -> Acc;
build_pairs([Key | Keys], Map, Order, Acc) ->
  Value = maps:get(Key, Map),
  KeyBin = encode_key(Key),
  Pair = [encode_value(KeyBin, Order), <<":">>, encode_value(Value, Order)],
  build_pairs(Keys, Map, Order, [Pair | Acc]).
...
encode_pretty(null, _Depth, _Indent, Order) ->
  encode_value(null, Order);
encode_pretty(true, _Depth, _Indent, Order) ->
  encode_value(true, Order);
encode_pretty(false, _Depth, _Indent, Order) ->
  encode_value(false, Order);
encode_pretty(Int, _Depth, _Indent, Order) when is_integer(Int) ->
  encode_value(Int, Order);
encode_pretty(Float, _Depth, _Indent, _Order) when is_float(Float) ->
  float_to_binary(Float, [{decimals, 15}, compact]);
encode_pretty(Bin, _Depth, _Indent, Order) when is_binary(Bin) ->
  encode_value(Bin, Order);
encode_pretty(Atom, _Depth, _Indent, Order) when is_atom(Atom) ->
  encode_value(Atom, Order);
...
build_pretty_pairs([Key | Keys], Map, Indent, Step, Order, Acc) ->
  Value = maps:get(Key, Map),
  KeyBin = encode_key(Key),
  Pair = [indent(Indent), encode_value(KeyBin, Order), <<": ">>,
